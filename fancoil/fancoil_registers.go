...
func tempToModbusValue(t *pb.Temperature) (uint16, error) {
	c := t.GetDegreesCelcius()
	cTenths := int(math.Round(float64(c * 10)))
	cTenthsAbs := cTenths
	signMask := uint16(0)
	if cTenthsAbs < 0 {
		cTenthsAbs *= -1
		signMask = negativeTempBit
	}
	if cTenthsAbs > int(tempNonSignMask) {
		return 0, fmt.Errorf("absolute value of temperature %f°C is out of range (> %f°C)", c, float64(tempNonSignMask)/10)
	}
	return signMask | uint16(cTenthsAbs), nil
}
